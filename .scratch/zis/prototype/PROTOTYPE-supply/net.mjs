...
// #11: #8 found GitHub releases 403 without auth — all nine repos returned 0
// items in #6's run and it went unremarked. An authed response is a different
// response, so it gets a different cache slot; otherwise the cached 403 wins
// forever and the fix is invisible.
function authSuffix(auth) {
  return auth ? `+auth${createHash('sha1').update(auth).digest('hex').slice(0, 6)}` : '';
}
...
export async function fetchWithPolicy(url, opts = {}) {
  const { method = 'GET', redirect = 'follow', maxBytes = 8 * 1024 * 1024, accept, auth } = opts;
  const f = join(CACHE, `${key(url, method + redirect)}${authSuffix(auth)}.json`);
  if (existsSync(f)) {
    stats.hits++;
